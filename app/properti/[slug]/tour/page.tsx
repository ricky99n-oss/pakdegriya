...
export default async function PublicTourPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [scenesRes, mediaRes] = await Promise.all([
    supabase
      .from("scenes")
      .select("id, name, is_first_scene, sort_order, initial_pitch, initial_yaw, media_id, audio_media_id, created_at")
      .eq("property_id", property.id)
      .order("sort_order", { ascending: true })
      .order("created_at", { ascending: true }),
    supabase.from("property_media").select("id, file_type").eq("property_id", property.id),
  ]);
  ...
  return <TourViewer tourConfig={tourConfig} introPlanetUrl={introPlanetUrl} />;
}
