...
import { daftarMemberAction, verifyHumanAction } from "../actions";
...
export default function HalamanDaftar() {
  ...
  const ensureHuman = async () => {
    ...
    if (!turnstileToken) { setErrorMsg("Selesaikan verifikasi keamanan terlebih dahulu."); return false; }
    const result = await verifyHumanAction(turnstileToken);
    if (!result.success) { setErrorMsg(result.error || "Verifikasi keamanan gagal."); setTurnstileToken(""); return false; }
    return true;
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res?.error) { setErrorMsg(res.error); setTurnstileToken(""); }
    else { setSuccessMsg("Pendaftaran berhasil! Silakan masuk ke akun Anda."); event.currentTarget.reset(); setTurnstileToken(""); }
    setIsLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FFF7E8] p-4 md:p-6 font-sans text-[#281C15]">
      <div className="max-w-5xl w-full bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row-reverse border border-[#D6A34A]/20">
        <div className="w-full md:w-1/2 p-8 md:p-14 flex flex-col justify-center">
          <div className="mb-8"><Link href="/" className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[#4A2F1B] text-[#D6A34A] font-bold text-3xl mb-6 shadow-md">P</Link><h1 className="text-3xl md:text-4xl font-black text-[#4A2F1B]">Daftar Member</h1><p className="text-[#281C15]/70 mt-2 font-medium">Buka akses eksklusif ke Virtual Tour 360° dan galeri properti lengkap.</p></div>
          ...
          <div className="mb-5"><TurnstileWidget onToken={setTurnstileToken} onExpire={() => setErrorMsg("Verifikasi keamanan kedaluwarsa. Silakan ulangi.")} /></div>
