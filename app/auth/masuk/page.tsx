...
import { masukAction, verifyHumanAction } from "../actions";
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FFF7E8] p-4 md:p-6 font-sans text-[#281C15]">
      <div className="max-w-5xl w-full bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row border border-[#D6A34A]/20">
        <div className="w-full md:w-1/2 p-8 md:p-14 flex flex-col justify-center">
          ...
          <div className="mb-5">
            <TurnstileWidget onToken={setTurnstileToken} onExpire={() => setErrorMsg("Verifikasi keamanan kedaluwarsa. Silakan ulangi.")} />
          </div>

          <button type="button" onClick={handleGoogle} disabled={isLoading || (turnstileConfigured && !turnstileToken)} className="w-full flex justify-center items-center gap-3 bg-white text-gray-700 font-bold py-3.5 rounded-xl border border-gray-300 hover:bg-gray-50 transition-all shadow-sm disabled:opacity-50">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div><label className="block text-sm font-bold text-[#281C15] mb-2">Alamat Email</label><input type="email" name="email" required autoComplete="email" className="w-full border border-gray-200 p-3.5 rounded-xl bg-gray-50 focus:outline-none focus:border-[#D6A34A] focus:ring-2 focus:ring-[#D6A34A]/20 transition-all text-[#281C15]" placeholder="budi@email.com" /></div>
            <div><label className="block text-sm font-bold text-[#281C15] mb-2">Kata Sandi</label><input type="password" name="password" required autoComplete="current-password" className="w-full border border-gray-200 p-3.5 rounded-xl bg-gray-50 focus:outline-none focus:border-[#D6A34A] focus:ring-2 focus:ring-[#D6A34A]/20 transition-all text-[#281C15]" placeholder="••••••••" /></div>
            <button type="submit" disabled={isLoading || (turnstileConfigured && !turnstileToken)} className="w-full flex justify-center items-center gap-2 bg-[#4A2F1B] text-white font-bold py-4 rounded-xl hover:bg-[#281C15] transition-all shadow-md mt-4 disabled:opacity-70"><LogIn size={18} /> {isLoading ? "Memproses..." : "Masuk Sekarang"}</button>
          </form>
        ...
        <div className="hidden md:block w-full md:w-1/2 relative bg-[#4A2F1B]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#4A2F1B] via-[#4A2F1B]/40 to-transparent flex flex-col justify-end p-12"><h2 className="text-4xl font-black text-white mb-3">Portal <span className="text-[#D6A34A]">Pakde Griya</span></h2><p className="text-white/80 text-base leading-relaxed font-medium">Akses aman untuk mengelola properti dan tur 360°.</p></div>
        </div>
      </div>
    </div>
  );
