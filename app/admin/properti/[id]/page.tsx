...
export default async function KelolaMediaProperti({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (mediaError) console.error("Gagal mengambil media properti:", mediaError);
    ...
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link
            href="/admin/properti"
            className="p-2 bg-white rounded-xl shadow-sm border border-[#D6A34A]/20 hover:bg-[#FFF7E8]"
            aria-label="Kembali ke daftar properti"
          >
            <ArrowLeft size={24} className="text-[#4A2F1B]" />
          </Link>
          <div>
            <h1 className="text-3xl font-extrabold text-[#281C15] flex items-center gap-3 flex-wrap">
              Edit Properti
              <span
                className={`text-xs px-2 py-1 rounded-md uppercase font-bold ${
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <Link
            ...
            className="px-6 py-2.5 rounded-xl font-bold bg-[#4A2F1B] text-[#D6A34A] hover:bg-[#281C15] border border-[#D6A34A]/50 text-sm"
          >
            Buka Editor Tur 360°
          </Link>
        ...
        <ActionForm action={updatePropertyAction} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Field label="Jenis Properti">
              <select name="propertyType" defaultValue={property.property_type} className={inputClass}>
          ...
          <div className="flex justify-end pt-4">
            <button type="submit" className="flex items-center gap-2 bg-[#D6A34A] text-[#281C15] font-bold py-2.5 px-6 rounded-xl hover:bg-[#c2913b] shadow-md">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[330px_1fr] gap-8 items-start">
        <aside className="bg-white p-6 rounded-2xl shadow-sm border border-[#D6A34A]/20 xl:sticky xl:top-6">
        ...
        <section className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-[#D6A34A]/20 min-h-full">
          <div className="mb-6">
            <h2 className="text-xl font-bold text-[#4A2F1B]">Media Tersimpan</h2>
            <p className="text-xs text-gray-400 mt-1">Media dipisahkan berdasarkan kategori. Status publik / privat dapat diubah per file.</p>
          </div>
          <MediaLibrary propertyId={property.id} mediaFiles={mediaFiles} />
        </section>
      </div>
    </div>
  );
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div><label className="block text-sm font-medium mb-1 text-[#281C15]">{label}</label>{children}</div>;
}

function MiniNumber({ label, name, value }: { label: string; name: string; value: number }) {
  return <div><label className="block text-xs font-bold mb-1 text-gray-500">{label}</label><input type="number" min="0" name={name} defaultValue={value} className={inputClass} /></div>;
}
