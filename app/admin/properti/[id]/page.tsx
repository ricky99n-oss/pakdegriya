...
import { ArrowLeft, ImagePlus, Lock, Globe, Save, Trash2, Music, Sparkles, AlertTriangle } from "lucide-react";
...
export default async function KelolaMediaProperti({ params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Menangkap Error Asli agar tidak muncul layar hitam kosong
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FFF7E8] p-6 text-center w-full">
        <div className="bg-white p-8 rounded-2xl shadow-xl border border-red-200 max-w-2xl w-full">
          <AlertTriangle className="mx-auto text-red-500 mb-4" size={48} />
          <h1 className="text-2xl font-bold text-red-600 mb-2">Gagal Memuat Halaman</h1>
          <p className="text-gray-600 text-sm mb-4">Terjadi kesalahan pada sistem (Internal Server Error).</p>
          <div className="bg-red-50 text-red-800 p-4 rounded-xl text-xs font-mono break-all text-left space-y-3 border border-red-100 overflow-auto max-h-96">
            <div>
              <span className="font-bold text-red-900 block mb-1">Pesan Error Asli:</span>
              {error.message || "Unknown Error"}
            </div>
            {error.stack && (
              <div className="pt-2 border-t border-red-200">
                <span className="font-bold text-red-900 block mb-1">Detail Trace:</span>
