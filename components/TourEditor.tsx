...
import { Crosshair, Target, Trash2, Layers } from "lucide-react";
import {
  createHotspotAction,
  deleteHotspotAction,
  ...
  useEffect(() => {
    ...
    try {
      viewerInstance.current = window.pannellum.viewer(viewerRef.current.id, {
        ...
        // Stream langsung dari R2 lewat route yang sekarang mendukung byte-range.
        // Jangan pakai ?buffer=true; gambar 360 resolusi tinggi bisa membuat Worker kehabisan memori.
      ...
      console.error("Pannellum editor gagal diinisialisasi:", error);
      setViewerError("Viewer 360 gagal dimuat. Coba refresh halaman atau cek file panorama.");
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Script
        ...
        onError={() => setViewerError("Library viewer 360 gagal dimuat.")}
      />
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#D6A34A]/20">
          ...
          <div className="flex flex-col xl:flex-row gap-6">
            <div className="w-full xl:w-2/3 h-[500px] relative bg-black rounded-xl overflow-hidden shadow-inner border-2 border-gray-200">
              ...
              {viewerError && (
                <div className="absolute inset-0 z-20 flex items-center justify-center p-6 bg-black/75 text-center">
                  <div className="max-w-md text-white">
                    <p className="font-bold mb-2">Viewer 360 bermasalah</p>
                    <p className="text-sm text-white/70">{viewerError}</p>
                  </div>
                </div>
              )}
            ...
            <div className="w-full xl:w-1/3 flex flex-col gap-4">
              <div className="bg-[#FFF7E8] p-5 rounded-xl border border-[#D6A34A]/30">
                ...
                <form action={handleHotspotSubmit} className="space-y-3">
                  ...
                  <div>
                    ...
                    <select name="iconType" className="w-full border border-[#D6A34A]/50 p-2.5 rounded-lg bg-white text-[#281C15] text-sm focus:outline-none">
                      <option value="door">🚪 Ikon Pintu Klasik</option>
                      <option value="arrow">⬆️ Panah Arah</option>
                      <option value="thumbnail">🖼️ Thumbnail Ruangan</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-[#281C15] mb-1">Nama Hotspot</label>
                    <input name="label" required className="w-full border border-[#D6A34A]/50 p-2.5 rounded-lg bg-white text-sm" placeholder="Contoh: Masuk Kamar" />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-[#281C15] mb-1">Tujuan Ruangan</label>
                    <select name="targetSceneId" required className="w-full border border-[#D6A34A]/50 p-2.5 rounded-lg bg-white text-sm">
                      <option value="">Pilih tujuan...</option>
                      {existingScenes.filter(s => s.id !== activeSceneId).map(scene => (
                        <option key={scene.id} value={scene.id}>{scene.name}</option>
                      ))}
                    </select>
                  </div>

                  <button type="submit" disabled={pitch === "" || yaw === ""} className="w-full bg-[#D6A34A] text-[#281C15] font-black py-3 rounded-lg disabled:opacity-50">
                    Simpan Hotspot
                  </button>
                </form>
              </div>
              ...
              {sceneHotspots.length > 0 && (
                <div className="bg-white p-4 rounded-xl border border-gray-200">
                  <h4 className="font-bold text-sm mb-3 text-[#4A2F1B]">Hotspot Ruangan Ini</h4>
                  <div className="space-y-2">
                    {sceneHotspots.map(h => (
                      <form action={deleteHotspotAction} key={h.id} className="flex items-center justify-between gap-3 bg-gray-50 rounded-lg p-2.5">
                        <input type="hidden" name="hotspotId" value={h.id} />
                        <input type="hidden" name="propertyId" value={propertyId} />
                        <span className="text-xs font-medium truncate">{(h.label || "Hotspot").split("|||")[0]}</span>
                        <button type="submit" className="text-red-600 hover:bg-red-100 rounded-md p-1.5" aria-label="Hapus hotspot">
                          <Trash2 size={15} />
                        </button>
                      </form>
                    ))}
              ...
              {availableAudios?.length > 0 && (
                <p className="text-[10px] text-gray-400 text-center">{availableAudios.length} audio tersedia untuk tour.</p>
              )}
            </div>
          </div>
        </div>
