...
import type { CoordinateValue, EditorHotspot, EditorScene, PannellumViewer } from "./tour360/types";
...
export default function TourEditor({ existingScenes, propertyId, allHotspots, availableAudios }: Props) {
  ...
  const currentScene = useMemo(() => existingScenes.find((scene) => scene.id === activeSceneId), [existingScenes, activeSceneId]);
  const sceneHotspots = useMemo(() => allHotspots.filter((hotspot) => hotspot.sceneId === activeSceneId), [allHotspots, activeSceneId]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      viewer.on("load", () => { setLoading(false); setViewerError(""); viewer?.resize(); });
      viewer.on("error", (message: unknown) => { setLoading(false); setViewerError(errorMessage(message, "Panorama gagal dimuat dari R2.")); });
  ...
  if (!currentScene) return <div className="bg-white border rounded-2xl p-8 text-center">Belum ada ruangan 360°.</div>;

  return (
    <div className="space-y-6">
      <PannellumAssets onReady={assetsReady} onError={assetsError} />
      <Tour360Styles />
      <TourEditorToolbar scenes={existingScenes} propertyId={propertyId} activeSceneId={activeSceneId} onSelectScene={selectScene} />
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#D6A34A]/20">
        ...
        <div className="flex flex-col xl:flex-row gap-6">
          <div className="w-full xl:w-2/3 h-[500px] relative bg-black rounded-xl overflow-hidden border-2 border-gray-200">
            ...
            {viewerError && <ErrorOverlay message={viewerError} onRetry={() => setRetryKey((key) => key + 1)} />}
...
function LoadingOverlay() {
  return <div className="absolute inset-0 z-20 bg-black flex flex-col items-center justify-center text-white"><Loader2 size={36} className="animate-spin text-[#D6A34A] mb-3" /><p className="font-bold text-sm">Memuat Panorama 360°</p><p className="text-xs text-white/50 mt-1">Streaming dari Cloudflare R2...</p></div>;
}

function ErrorOverlay({ message, onRetry }: { message: string; onRetry: () => void }) {
  return <div className="absolute inset-0 z-30 bg-black/95 flex items-center justify-center p-8"><div className="text-center max-w-md"><AlertTriangle size={38} className="text-red-400 mx-auto mb-3" /><p className="text-white font-bold">Viewer 360 Gagal Dimuat</p><p className="text-white/60 text-xs mt-2 break-words">{message}</p><button type="button" onClick={onRetry} className="mt-5 bg-[#D6A34A] text-[#281C15] px-5 py-2 rounded-lg text-xs font-bold">Coba Muat Ulang</button></div></div>;
}
