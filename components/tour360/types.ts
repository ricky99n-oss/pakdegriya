...
export type PannellumGlobal = { viewer: (container: HTMLElement | string, config: Record<string, any>) => PannellumViewer };

declare global { interface Window { pannellum?: PannellumGlobal; } }
...
export type EditorHotspot = {
  id: string; sceneId: string; targetSceneId: string; pitch: number | string;
  yaw: number | string; label?: string | null; [key: string]: any;
};

export type PublicHotspot = {
  pitch: number | string; yaw: number | string; type?: string; text?: string;
  sceneId?: string; [key: string]: any;
};
...
export type PublicTourConfig = {
  default?: {
    firstScene?: string; sceneFadeDuration?: number; autoLoad?: boolean;
    autoRotate?: number; autoRotateInactivityDelay?: number; [key: string]: any;
  };
