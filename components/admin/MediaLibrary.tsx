...
type Props = { propertyId: string; mediaFiles: MediaItem[] };
...
export default function MediaLibrary({ propertyId, mediaFiles }: Props) {
  if (!mediaFiles.length) {
    return <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-300"><ImageIcon className="mx-auto text-gray-300 mb-3" size={34} /><p className="text-gray-500 font-medium">Belum ada media yang diunggah.</p></div>;
  }
          ...
          <section key={group.key} className="rounded-2xl border border-[#D6A34A]/20 overflow-hidden bg-white">
            <div className="px-4 md:px-5 py-4 bg-[#FFF9EF] border-b border-[#D6A34A]/15 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-9 h-9 rounded-xl bg-[#D6A34A]/15 text-[#9B6A1A] flex items-center justify-center shrink-0"><Icon size={18} /></div>
                <div className="min-w-0"><h3 className="font-black text-[#4A2F1B] text-sm md:text-base">{group.title}</h3><p className="text-[10px] md:text-xs text-gray-400 truncate">{group.description}</p></div>
              </div>
            ...
            <div className="p-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">{items.map((media) => <MediaCard key={media.id} propertyId={propertyId} media={media} />)}</div>
          </section>
        );
      })}
    </div>
      ...
      <div className="relative aspect-[16/10] bg-gray-100 overflow-hidden">
        ...
        <div className="absolute top-2 left-2">
          <span className={`backdrop-blur text-white text-[10px] font-black px-2.5 py-1.5 rounded-full flex items-center gap-1.5 shadow ${isPublic ? "bg-emerald-500/95" : "bg-red-500/95"}`}>
            {isPublic ? <Globe2 size={11} /> : <Lock size={11} />}{isPublic ? "Publik" : "Privat"}
          </span>
        </div>
      </div>
