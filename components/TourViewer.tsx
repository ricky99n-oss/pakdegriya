...
type Props = { tourConfig: PublicTourConfig; introPlanetUrl?: string };
...
export default function TourViewer({ tourConfig, introPlanetUrl }: Props) {
  ...
  const pannellumScenes = useMemo(() => Object.fromEntries(sceneIds.map((id) => {
    ...
    const hotSpots = (scene.hotSpots || [])
      ...
      .map((hotspot) => {
        ...
        return {
          ...
          createTooltipArgs: { label: label || target?.title || "Pindah Ruangan", iconType, targetImage: target?.thumbnail || "", animated: true },
        };
      });
    return [id, {
  ...
  const playSceneAudio = useCallback((sceneId: string) => {
    ...
    if (audio.getAttribute("src") !== source) { audio.pause(); audio.src = source; audio.load(); }
  ...
  useEffect(() => {
    ...
    return () => { audio.pause(); audio.removeAttribute("src"); document.removeEventListener("fullscreenchange", onFullscreen); audioRef.current = null; };
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (typeof ResizeObserver !== "undefined") { observer = new ResizeObserver(() => viewer?.resize()); observer.observe(viewerRef.current); }
    } catch (error) {
      setLoading(false);
      setViewerError(errorMessage(error));
    }
    return () => { observer?.disconnect(); try { viewer?.destroy(); } catch {} if (viewerInstance.current === viewer) viewerInstance.current = null; };
  ...
  const startTour = (withAudio: boolean) => {
    ...
    // Jangan memaksa HFOV ke 90 saat intro ditutup. Scene mempertahankan HFOV 120
    // sehingga tampilan awal lebih lebar dan tidak terasa ter-zoom.
  ...
  const toggleAudio = () => {
    if (audioEnabledRef.current && isAudioPlaying) { audioEnabledRef.current = false; audioRef.current?.pause(); setIsAudioPlaying(false); }
    else { audioEnabledRef.current = true; playSceneAudio(currentSceneId); }
  };
  ...
  return (
    <div ref={viewerContainerRef} className="w-full h-screen bg-black relative overflow-hidden font-sans">
      ...
      <TourViewerUI
        tourConfig={tourConfig} introPlanetUrl={introPlanetUrl} currentSceneId={currentSceneId}
        started={started} loading={loading} error={viewerError} showTools={showTools}
        showGallery={showGallery} showGuide={showGuide} isFullscreen={isFullscreen} isAudioPlaying={isAudioPlaying}
